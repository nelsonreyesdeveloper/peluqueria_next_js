...
const Servicio = ({ servicio, admin = false }) => {
    ...
    const { handleAgregarServicio, handleDeleteServicioAdmin,
        handleModal, modalIsOpen, setServicioModal, handleEditarServicio } = usePeluqueriaContext()

    const { id, nombre, precio, descripcion, text_imagen, estado, marcado } = servicio
                        ...
                        <div className='flex h-full justify-between flex-col'>
                            <div>
                                ...
                                <div className="px-5 pb-5">
                                    <h5 className={`text-2xl uppercase font-semibold ${marcado ? 'text-white' : ''}  tracking-tight text-gray-900 dark:text-white `}>{nombre}</h5>
                                    <p className={`line-clamp-2 ${marcado ? 'text-white' : ''}`}>{descripcion}</p>
                                    ...
                                    <div className="flex items-center justify-center">
                                        <span className={`text-3xl font-bold ${marcado ? 'text-white' : ''} text-center text-gray-900 dark:text-white`}>{formatearDinero(precio)}</span>
                                    </div>
